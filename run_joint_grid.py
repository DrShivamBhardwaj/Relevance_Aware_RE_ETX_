...
from wsn_hfl.config import SimConfig, TUNING_SEEDS
...
def main():
 ...
 if a.dataset=='intel':
  ...
  fn=lambda c:simulate_intel('proposed',c,ROOT/'data/intel_lab',.9)[0];out=ROOT/'results/intel_lab/joint_grid.csv'
  metrics=['rmse_c','effective_bits','energy_j','max_relay_energy_j','utility_target_js','temperature_coverage_js','participation_jain']
 else:
  ...
  fn=lambda c:simulate_har('proposed',c,ROOT/'data/uci_har',.9)[0];out=ROOT/'results/uci_har/joint_grid.csv'
  metrics=['accuracy','macro_f1','effective_bits','energy_j','max_relay_energy_j','utility_target_js','class_coverage_js','participation_jain']
 rows=[];t=time.time()
 for rp in RPS:
  for beta in BETAS:
   runs=[fn(replace(base,seed=s,relay_pressure_weight=rp,compression_distortion_weight=beta)) for s in SEEDS]
   o={'relay_pressure_weight':rp,'compression_distortion_weight':beta,'n':len(runs),'seed_role':'tuning'}
   for k in metrics:
