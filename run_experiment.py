...
def aggregate(rows):
    numeric = [
        ...
        "participation_jain", "utility_target_js", "class_coverage_js",
...
def main():
    ...
    cfg = SimConfig()
    if args.quick:
        cfg = replace(cfg,rounds=8,num_clients=12,clients_per_round=4,samples_per_client=48,test_samples=400)

    out_dir = ROOT / args.out
    raw_rows=[];history_rows=[];aggregate_rows=[]
    for corr in args.correlations:
        for method in args.methods:
            ...
            for seed,hist in zip(args.seeds,histories):
                for row in hist:history_rows.append({"seed":seed,"correlation":corr,**row})
            ...
            print(
                ...
                f"utilityJS={aggregate_rows[-1]['utility_target_js_mean']:.4f}"
            )
    write_csv(out_dir/"raw_summary.csv",raw_rows);write_csv(out_dir/"round_history.csv",history_rows);write_csv(out_dir/"aggregate_summary.csv",aggregate_rows)
    manifest={"methods":args.methods,"correlations":args.correlations,"seeds":args.seeds,"seed_role":"held-out evaluation","quick":args.quick,"config":cfg.__dict__}
    out_dir.mkdir(parents=True,exist_ok=True)
    with (out_dir/"run_manifest.json").open("w") as f:json.dump(manifest,f,indent=2)
